function [z,model] = mdsInit(D,r,options)
% function of multi dimention scalling  (MARK-III)
%
% Synopsis:
% [Z,model] = MDS(D,r)
% [Z,model] = MDS(D,r,options)
%
% Description:
...
%  r        scale value         indicates the dimension of the new system
%
% Output:
...
if r >= 1   %calculated first r bases.
    [U,D] = eigs(D_Dc, r);			% U is the a in paper
%     [u,s,v] = svds(y,r);
...
elseif r<1   
    [U,D] = eig(D_Dc);			% U is the a in paper
...
z = U(:,1:rank) * diag(sqrt(eigenValue(1:rank)));     

% [Evec,Eval]=eig(D_Dc);
% [Evec,Eval]=eigs(D_Dc,r);
...
% z=vec(:,1:r)*sqrt(abs(val(1:r,1:r)));     
