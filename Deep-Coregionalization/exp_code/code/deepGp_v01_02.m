function [Ypred, model] = deepGp_v01_02(xtr,Ytr,xte, iMethod)
% v01: simple deep GP.
% v01_02: same model as v01 but using GP with normalize y x
...
switch iMethod
    case 1 % independent GP
        for k = 1:nLv
            ...
            kxtr = xtr(1:nSample,:);            
            model{k} = cigp_v2_02(kxtr, Ytr{k}, xte);
    ...
    case 2 % simple concatinate structure
        ...
        kxtr = xtr(1:nSample,:);  
        model{1} = cigp_v2_02(kxtr, Ytr{1}, xte);
        Ypred{1} = model{1}.yTe_pred;
        for k = 2:nLv
%             model{k} = cigp_v2_02(kxtr, Ytr{k}, xte);nSample = size(Y{k}, 1);
            nSample = size(Ytr{k}, 1);
            kxte = model{k-1}.yTe_pred;
            
            kxtr = Ytr{k-1}(1:nSample,:);
            model{k} = cigp_v2_02(kxtr, Ytr{k}, kxte);
            Ypred{k} = model{k}.yTe_pred;
        end
    ...
    case 3 % residual deep GP. case 2 plus x
        ...
        kxtr = xtr(1:nSample,:);  
        model{1} = cigp_v2_02(kxtr, Ytr{1}, xte);        
%         model{1} = cigp_v2_02(xtr, Ytr{1}, xte);
        Ypred{1} = model{1}.yTe_pred;
        for k = 2:nLv
%             model{k} = cigp_v2_02(kxtr, Ytr{k}, xte);nSample = size(Y{k}, 1);
            nSample = size(Ytr{k}, 1);
