...
class IFC2dGPT(nn.Module):
    ...
    def _eval_elbo_llh_t(self, ht, yt, tt):
        ...
        fid_t = self.dataset.func_t_to_fid(tt.item())
        tid = self.dataset.t_list_tr.index(tt.item())

        N = yt.shape[0]

        yt_2d = yt.reshape([N, fid_t, fid_t]).unsqueeze(1)
