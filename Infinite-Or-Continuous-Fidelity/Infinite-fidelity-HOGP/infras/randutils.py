...
def _generate_boundary_inputs(N, lb, ub, seed=None):
    rand_state = np.random.get_state()
    ...
    try:
        ...
        for i in range(N):
            ...
            fixed_dims = np.sort(np.random.choice(dim, size=2, replace=False)).tolist()
            ...
            for d in range(dim):
                if d in fixed_dims:
                    ...
                    xsd = np.random.uniform(low=lb[d], high=ub[d])
                    xs.append(xsd)
                #
...
def generate_with_bounds(N, lb, ub, method='uniform', seed=None):
    ...
    if method == 'uniform':
        ...
        X = _generate_boundary_inputs(N, lb, ub, seed)
    else:
