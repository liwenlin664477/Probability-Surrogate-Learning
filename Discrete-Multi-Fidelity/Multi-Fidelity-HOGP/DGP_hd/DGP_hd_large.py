...
class DGP_hd:
    ...
    def __init__(self, cfg, jitter):
        ...
        # phi
        self.phi = [tf.compat.v1.placeholder(tf.float64, [self.n_samples[i], self.n_bases[i]]) for i in
                    range(self.n_layers)]
        # U
        # M, L, R
    ...
    def train(self):
        ...
        for i in range(self.epoch):
            data_dict.update({self.phi[i]: np.random.normal(size=[self.n_samples[i], self.n_bases[i]]) for i in
                              range(self.n_layers)})
