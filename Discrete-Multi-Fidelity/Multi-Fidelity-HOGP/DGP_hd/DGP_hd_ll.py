...
class DGP_hd:
    ...
    def train(self):
        ...
        return self.S0R0, self.S0R1, self.S1R0, self.S1R1, S0LL, S1LL
    ...
    def sample(self, flag):
        ...
        cov = tf.matmul(tf.transpose(B_i), tf.matmul(kBB, B_i)) + tf.exp(-log_tau_i) * tf.eye(self.out_d,
                                                                                              dtype=util.tf_type)
        # Y_ll = -0.5 * self.out_d * np.log(2 * np.pi) - 0.5 * tf.linalg.logdet(cov) - 0.5 * self.trace(tf.linalg.solve(cov, tf.transpose(Y_pre)), Y_pre) / tf.cast(tf.shape(Y_pre)[0], util.tf_type)
        cov = tf.reshape(tf.linalg.tensor_diag_part(cov), (1, -1))
        Y_ll = -0.5 * np.log(2 * np.pi) - 0.5 * tf.reduce_sum(tf.math.log(cov)) / self.out_d - 0.5 * tf.reduce_mean(
            tf.square(self.Y_test) / cov)
        return Y_pre, Y_ll
