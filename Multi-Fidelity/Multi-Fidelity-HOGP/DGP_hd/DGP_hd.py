...
class DGP_hd:
    ...
    def __init__(self, cfg, jitter):
        ...
        # input
        self.X = [tf.compat.v1.placeholder(tf.float64, [self.n_samples[i], self.in_d]) for i in range(self.n_layers)]
        # output
        self.Y = [tf.compat.v1.placeholder(tf.float64, [self.n_samples[i], self.out_d]) for i in range(self.n_layers)]
        # phi
        ...
        for i in range(self.n_layers):
            self.M.append(tf.Variable(self.feed_data['M'][i], dtype=tf.float64))
            # self.M.append(tf.Variable(tf.zeros([self.n_samples[i], self.n_bases[i]], dtype=tf.float64), dtype = tf.float64))
            # self.M.append(tf.Variable(np.random.rand(self.n_samples[i], self.n_bases[i]), dtype = tf.float64))
            # self.M.append(tf.Variable(np.zeros([self.n_samples[i], self.n_bases[i]]), dtype=tf.float64))
            # self.L.append(tf.Variable(self.feed_data['L'][i], dtype=tf.float64))
            # self.R.append(tf.Variable(self.feed_data['R'][i], dtype=tf.float64))
            ...
            for j in range(n_bases):
                # print(j)
                U_ij = [tf.Variable(self.feed_data['U'][i][j][k].reshape([1, -1]), dtype=tf.float64) for k in
                        range(len(self.dim))]
                # U_ij = [tf.Variable(np.random.rand(self.n_samples[i], self.n_bases[i]), dtype=tf.float64) for k in self.dim]
                U_i.append(U_ij)
            self.U.append(U_i)
        ...
        # test
        self.X_test = tf.compat.v1.placeholder(tf.float64, [None, self.in_d])
    ...
    def train(self):
        ...
        for i in range(self.epoch):
            ...
            N_Y_pre_s0, N_Y_pre_s1 = self.fit()
    ...
    def sample(self, flag):
        ...
        for i in range(self.n_layers):
            ...
            if flag[i] == 0:
                ...
                phi = tf.random.normal([self.n_samples[i], self.n_bases[i]], dtype=tf.float64)
            ...
            if flag[i] == 1:
                ...
                phi = tf.random.normal([tf.shape(self.X_test)[0], self.n_bases[i]], dtype=tf.float64)
