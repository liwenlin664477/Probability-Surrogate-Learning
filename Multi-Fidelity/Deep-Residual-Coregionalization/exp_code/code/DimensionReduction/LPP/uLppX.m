function [Z,model] = uLppX(X,options)
% uLppX: Uncorrelated Locality Preserving Projections by XING
%
% Synopsis:
% [Z,model] = LppX(X)
...
if ~isfield(options,'t'),options.t = 2; end                             % Default parameter for heat kernel=3;
...
mu=mean(X,2);
C=(X*X')./num-mu*mu';            % Covariance matrix
% C=((X-(repmat(mu,1,num)))*(X-(repmat(mu,1,num)))')./num;    % Another way to calculate C with same result.
...
for k = 2:options.dim_new
    
%     M=inv(V(:,1:k-1)'*C*inv(S_D)*V(:,1:k-1))*V(:,1:k-1)'*C*inv(S_D)*C*V(:,1:k-1)-inv(V(:,1:k-1)'*inv(S_D)*V(:,1:k-1))*V(:,1:k-1)'*inv(S_D)*C*V(:,1:k-1);
%     N=inv(V(:,1:k-1)'*C*inv(S_D)*V(:,1:k-1))*V(:,1:k-1)'*C*inv(S_D)-inv(V(:,1:k-1)'*inv(S_D)*V(:,1:k-1))*V(:,1:k-1)'*inv(S_D);
%     Q=inv(V(:,1:k-1)'*C*inv(S_D)*C*V(:,1:k-1))*V(:,1:k-1)'*C*inv(S_D)*V(:,1:k-1)-inv(V(:,1:k-1)'*inv(S_D)*C*V(:,1:k-1))*V(:,1:k-1)'*inv(S_D)*V(:,1:k-1);
%     R=inv(V(:,1:k-1)'*C*inv(S_D)*C*V(:,1:k-1))*V(:,1:k-1)'*C*inv(S_D)-inv(V(:,1:k-1)'*inv(S_D)*C*V(:,1:k-1))*V(:,1:k-1)'*inv(S_D);   
%     P_k=eye(dim)-C*V(:,1:k-1)*inv(M)*N-V(:,1:k-1)*inv(Q)*R;
    
       
    P_k=eye(dim)-C*(V(:,1:k-1)*inv(V(:,1:k-1)'*C*inv(S_D)*C*V(:,1:k-1))*V(:,1:k-1)'*C)*inv(S_D);

    PkSL=P_k*S_L;
    

    [eigvector, eigvalue] = eig(PkSL, S_D);
    [eigvalue, ind] = sort(diag(eigvalue), 'descend');
%     [eigvalue, ind] = sort(diag(eigvalue), 'ascend');
